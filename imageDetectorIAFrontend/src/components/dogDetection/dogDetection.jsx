...
export const DogDetection = () => {
  //STORE WITH ZUSTAND
  const { isDog, isDetecting, result, loading, imageURL } = useImageStore();
      ...
      <div className="p-4 rounded-lg w-[50%] flex flex-col items-center">
        ...
        {isDetecting && (
          <p className="text-white font-bold my-5">
            this image has already been detected
          </p>
        )}
        <button
          className={`${!loading && "bg-slate-200 rounded-full flex mt-10"}`}
          disabled={loading || result || isDetecting}
          onClick={handleDogDetection}
        >
